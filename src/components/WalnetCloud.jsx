      ...
      <div className="flex space-x-4">
        ...
        {currentStep < steps.length && (
          <button
            ...
            disabled={
              (currentStep === 1 && !selectedClusterType) ||
          ...
          <div className="w-80 border-r border-gray-500 pr-6 pl-8 h-full">
            ...
            <div className="space-y-4 relative">
              <div
                ...
                style={{
                  height: `${((currentStep - 1) / steps.length) * 100 + 10}%`,
                }}
              ></div>
            ...
            {currentStep === 2 && (
              <div>
                <div className="mb-6 flex flex-col lg:flex-row items-center gap-4">
                  ...
                  >
                    {selectedLocations.length === countries.length
                      ? "Deselect All"
                      : "Select All"}
                  </button>
                </div>
                    ...
                    >
                      <div className="flex items-center">
                        {processor.type === "apple" ? (
                          ...
                          <img
                            src={"/img/icon/nvidia.png"}
                            className="text-white mr-3 w-6 h-auto"
                          />
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2 bg-gray-800 bg-opacity-50 p-2 px-4 rounded">
                      <span>
                        {clusterTypes.find((p) => p.id === selectedClusterType)
                          ?.name || "Not selected"}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedLocations.slice(0, 5).map((locationCode) => {
                        const country = countries.find(
                          (c) => c.code === locationCode
                        );
                        return (
                          <div
                            key={locationCode}
                            className="flex items-center space-x-2 bg-gray-800 bg-opacity-50 px-3 py-1 rounded text-sm"
                          >
                            <ReactCountryFlag
                  ...
                  <div>
                    ...
                    <div className="bg-gray-800 bg-opacity-50 p-2 px-4 rounded flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        {selectedProcessor && (
                          <>
                            {processors.find((p) => p.id === selectedProcessor)
                              ?.type === "apple" ? (
                              <FaApple className="text-white w-4 h-4" />
                            ) : (
                              <img
                                src={"/img/icon/nvidia.png"}
                                className="text-white w-6 h-auto mt-1"
                              />
                            )}
                            <span>
                              {processors.find(
                                (p) => p.id === selectedProcessor
                              )?.name || "Not selected"}
                            </span>
                          </>
                        )}
                      ...
                      {selectedProcessor && (
                        <span className="text-gray-400">
                          {processors.find((p) => p.name === selectedProcessor)
                            ?.count || 0}
                        </span>
                      )}
                    </div>
